...
// ─── Info descriptions for every block ───────────────────────────────────────
const INFO: Record<string, string> = {
  "Realization Rate":
    "Billed hours ÷ worked hours × 100. Measures how much of your team's time is actually getting invoiced. ≥100% = excellent, 85–99% = good, <85% = leakage.",
  "Billable Utilization":
    "Billable hours ÷ total tracked hours × 100. Shows what % of time is spent on client work vs. internal/admin. Target: 75–85% for most CPA firms.",
  "WIP Pipeline":
    "Total dollar value of approved time blocks that haven't been invoiced yet. High WIP means cash is sitting on the table — time to bill.",
  "Effective Rate":
    "Total billing amount ÷ total billable hours. Your actual blended rate. Compare to your standard rate to see if discounts or write-downs are eroding revenue.",
  "Timesheet Compliance":
    "% of timesheets submitted within 2 days of week-end. Late submissions delay approvals, invoicing, and cash flow.",
  "Invoice Cycle Time":
    "Average days from timesheet approval to invoice creation. Shorter = faster cash collection. Target: under 7 days.",
  "Revenue Trend":
    "Monthly invoiced revenue. Reflects actual billed amounts — only as accurate as your imported invoices.",
  "WIP Aging":
    "Approved, uninvoiced work bucketed by age. 61–90+ day buckets are at risk of write-off — prioritize invoicing these first.",
  "Timesheet Compliance Chart":
    "Month-by-month compliance trend. Dips often correlate with busy season, staff changes, or onboarding.",
  "Realization Rate by Client":
    "Per-client realization: invoiced hours ÷ worked hours. Clients below 85% are being under-billed — review scope creep or write-down habits.",
  "Client Profitability":
    "Revenue (from billing_amount on time blocks) minus labor cost (hours × employee cost rate). All clients appear here even without imported invoices.",
  "Staff Utilization":
    "Hours per staff member: billable vs. non-billable. Low billable % may indicate admin overload or available capacity.",
  "Staff Detail":
    "Per-person breakdown of billable hours, total hours, and utilization rate. Useful for capacity planning and performance reviews.",
  "Invoice Revenue vs. Labor Cost":
    "Actual invoiced revenue vs. labor cost to deliver it. Uses real Invoice amounts — not estimated billing. Requires imported invoices to populate.",
  "Invoice Profitability by Client":
    "True margin per client: invoiced amount minus worked labor cost. Realization shows invoiced hours ÷ worked hours — gaps indicate unbilled time or scope creep.",
  "Hours by Service Line":
    "Hours distributed across service types (Tax, Bookkeeping, Advisory, etc.) per client. Requires task types to be tracked on time blocks.",
};
  ...
  return (
    ...
    >
      <div style={{
        ...
        border: `1px solid ${visible ? C.gold : C.slate}`,
        display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: 10, color: visible ? C.gold : C.slate,
        cursor: "help", fontFamily: "'DM Mono', monospace",
...
async function fetchDashboard(apiBase: string, period: string): Promise<DashboardResponse> {
  ...
  const res = await fetch(url, { headers: { Authorization: `Bearer ${_getToken()}`, "Content-Type": "application/json" } });
  if (!res.ok) {
...
function mapRevenueTrend(kpi: Partial<RevenueTrendKPI>): RevenueChartRow[] {
  return (kpi?.trend ?? []).map((t) => ({ month: t.month_label || t.month, revenue: t.revenue, invoices: t.invoice_count }));
}
function mapWipBuckets(kpi: Partial<WipKPI>): WipChartRow[] {
  if (!kpi?.buckets) return [];
  return [{ name: "0–30 days", value: kpi.buckets["0_30"] || 0 }, { name: "31–60 days", value: kpi.buckets["31_60"] || 0 }, { name: "61–90 days", value: kpi.buckets["61_90"] || 0 }, { name: "90+ days", value: kpi.buckets["90_plus"] || 0 }];
}
function mapClientRealization(kpi: Partial<RealizationKPI>): RealizChartRow[] {
  return [...(kpi?.clients ?? [])].sort((a, b) => b.realization - a.realization).slice(0, 10).map((c) => ({ name: c.client_name, realization: c.realization, status: c.status }));
}
function mapStaffUtilization(kpi: Partial<UtilizationKPI>): StaffChartRow[] {
  return (kpi?.staff ?? []).slice(0, 8).map((s) => ({ name: s.name.split(" ")[0], billable: parseFloat(s.billable_hours.toFixed(1)), nonBillable: parseFloat((s.total_hours - s.billable_hours).toFixed(1)), utilization: s.utilization }));
}
...
function mapProfitability(kpi: Partial<ProfitabilityKPI>): ProfitTableRow[] {
  return (kpi?.clients ?? []).slice(0, 8).map((c) => ({ name: c.client_name, revenue: c.revenue, cost: c.cost, margin: c.margin, marginPct: c.margin_pct, hours: c.hours }));
}

// ─── KPI Card ─────────────────────────────────────────────────────────────────
interface KPICardProps { title: string; value: string | number | null | undefined; unit?: string; sub?: string; delta?: number; color?: string; loading: boolean; }
...
function InfoCard({ children, infoKey, style }: { children: ReactNode; infoKey: string; style?: CSSProperties }) {
...
function ErrorBox({ kpiName, error }: { kpiName: string; error?: string }) {
  ...
  return <div style={{ padding: "12px 16px", background: "rgba(251,113,133,0.1)", border: `1px solid ${C.rose}`, borderRadius: 8, color: C.rose, fontSize: 13 }}>⚠ {kpiName}: {error}</div>;
}

interface CustomTooltipProps extends TooltipProps<number, string> { prefix?: string; suffix?: string; }
...
const PERIODS: { label: string; value: PeriodValue }[] = [{ label: "3 months", value: "3m" }, { label: "6 months", value: "6m" }, { label: "12 months", value: "12m" }, { label: "YTD", value: "ytd" }];

function buildStyles() {
  const periodBtn = (active: boolean): CSSProperties => ({ padding: "6px 14px", borderRadius: 6, border: `1px solid ${active ? C.gold : C.border}`, background: active ? "rgba(201,168,76,0.15)" : "transparent", color: active ? C.gold : C.slate, cursor: "pointer", fontSize: 12, letterSpacing: 1, fontFamily: "'DM Mono', monospace", transition: "all 0.15s" });
  const tabStyle = (active: boolean): CSSProperties => ({ padding: "10px 20px", cursor: "pointer", fontSize: 12, letterSpacing: 1, color: active ? C.gold : C.slate, borderBottom: `2px solid ${active ? C.gold : "transparent"}`, background: "transparent", border: "none", fontFamily: "'DM Mono', monospace", marginBottom: -1, transition: "all 0.15s" });
  return {
...
export default function ExecutiveDashboard({ apiBase = "https://timetracker-api-k375.onrender.com" }: { apiBase?: string }) {
  ...
  const OverviewTab = () => (
    <>
      ...
      <InfoCard infoKey="Revenue Trend">
        {loading ? <div style={{ height: 260, background: C.navyLt, borderRadius: 8 }} /> : (
          <ResponsiveContainer width="100%" height={260}>
            <AreaChart data={mapRevenueTrend(revTrend)} margin={{ top: 10, right: 20, left: 10, bottom: 0 }}>
              <defs>
        ...
        <InfoCard infoKey="WIP Aging">
          ...
          {loading ? <div style={{ height: 200, background: C.navyLt, borderRadius: 8 }} /> : (
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={mapWipBuckets(wip)} cx="50%" cy="50%" innerRadius={55} outerRadius={80} paddingAngle={3} dataKey="value">
                  {mapWipBuckets(wip).map((_, i) => <Cell key={i} fill={WIP_COLORS[i % WIP_COLORS.length]} />)}
                </Pie>
  ...
  // ── Clients ───────────────────────────────────────────────────────────────
  const ClientsTab = () => {
    const realizData = mapClientRealization(realiz);
    const profitData = mapProfitability(profit);
    return (
      <>
        <SectionTitle infoKey="Realization Rate by Client">Realization Rate by Client</SectionTitle>
        <InfoCard infoKey="Realization Rate by Client">
          {loading ? <div style={{ height: 280, background: C.navyLt, borderRadius: 8 }} /> :
           realizData.length === 0 ? <p style={{ color: C.slate, textAlign: "center", padding: 40 }}>No invoice data for this period. Import invoices via Settings → Invoices.</p> : (
            <ResponsiveContainer width="100%" height={realizData.length * 40 + 40}>
              <BarChart data={realizData} layout="vertical" margin={{ left: 20, right: 40 }}>
                ...
                <Bar dataKey="realization" name="Realization" radius={[0, 4, 4, 0] as [number, number, number, number]}>
                  {realizData.map((d, i) => <Cell key={i} fill={STATUS_COLOR[d.status] ?? C.gold} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          )}
        ...
        <InfoCard infoKey="Client Profitability">
          {loading ? <div style={{ height: 200, background: C.navyLt, borderRadius: 8 }} /> : (
            <table style={s.table}>
                    ...
                    <td style={s.td}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <div style={{ flex: 1, height: 6, background: C.navyLt, borderRadius: 3 }}>
                          <div style={{ width: `${Math.min(100, Math.max(0, r.marginPct))}%`, height: "100%", background: r.marginPct >= 40 ? C.teal : r.marginPct >= 20 ? C.gold : C.rose, borderRadius: 3 }} />
                        </div>
  ...
  const StaffTab = () => (
    <>
      ...
      <InfoCard infoKey="Staff Utilization">
        {loading ? <div style={{ height: 280, background: C.navyLt, borderRadius: 8 }} /> : (
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={mapStaffUtilization(utiliz)} margin={{ top: 10, right: 20 }}>
              <CartesianGrid strokeDasharray="3 3" stroke={C.border} />
  ...
  const BillingTab = () => {
    ...
    const barData = clients.map((c) => ({ name: c.client_name.length > 14 ? c.client_name.slice(0, 13) + "…" : c.client_name, fullName: c.client_name, invoiced: c.invoiced_amount, cost: c.worked_cost, margin: c.margin }));

    return (
      <>
        <SectionTitle infoKey="Invoice Revenue vs. Labor Cost">Invoice Revenue vs. Labor Cost</SectionTitle>
        <InfoCard infoKey="Invoice Revenue vs. Labor Cost">
          {loading ? <div style={{ height: 280, background: C.navyLt, borderRadius: 8 }} /> :
        ...
        <InfoCard infoKey="Invoice Profitability by Client">
          {loading ? <div style={{ height: 200, background: C.navyLt, borderRadius: 8 }} /> :
           clients.length === 0 ? <p style={{ color: C.slate, textAlign: "center", padding: 40 }}>No data for this period.</p> : (
            <table style={s.table}>
              <thead><tr>{["Client", "Invoiced", "Hours Worked", "Labor Cost", "Margin", "Margin %", "Realization", "Invoices"].map((h) => <th key={h} style={s.th}>{h}</th>)}</tr></thead>
              <tbody>
                {clients.map((c) => (
                  <tr key={c.client_id}>
        ...
        {clients.some((c) => c.services?.length > 1) && (
          <>
            ...
            <InfoCard infoKey="Hours by Service Line">
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={clients.filter((c) => c.worked_hours > 0).map((c) => { const row: Record<string, string | number> = { name: c.client_name }; c.services.forEach((sv) => { row[sv.service] = sv.hours; }); return row; })} margin={{ top: 10, right: 20 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke={C.border} />
                  <XAxis dataKey="name" tick={{ fill: C.white, fontSize: 11 }} />
                  <YAxis tick={{ fill: C.slate, fontSize: 11 }} tickFormatter={(v: number) => `${v}h`} />
                  <Tooltip content={<CustomTooltip suffix="h" />} />
                  <Legend wrapperStyle={{ fontSize: 12, color: C.slate }} />
                  {Array.from(new Set(clients.flatMap((c) => c.services.map((sv) => sv.service)))).map((svc, i) => (
    ...
    <div style={s.root}>
      ...
      <div style={s.tabs}>
        {(["overview", "clients", "billing", "staff"] as const).map((id) => (
          <button key={id} style={s.tab(tab === id)} onClick={() => setTab(id)}>{id.toUpperCase()}</button>
        ))}
      </div>
