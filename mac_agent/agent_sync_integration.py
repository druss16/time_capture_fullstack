...
class AgentSync:
    """
    Simple sync manager for TimeTracker Mac agent.
    ...
    def __init__(self, api_base: str, device_token: str):
        ...
        self.gui_menu_bar = None  # Set by run_agent() after GUI init
        ...
        # Load from disk cache on startup (instant client list before first network sync)
    ...
    def _check_and_sync(self):
        """Check for changes, sync only if hashes changed"""
        try:
    ...
    def _get(self, endpoint: str) -> Optional[Dict]:
        ...
        CRITICAL: uses DeviceKey auth, NOT Bearer.
        Bearer was a bug that caused all sync calls to silently 401/403,
        meaning clients never loaded from the backend on Mac.
        """
        url = f"{self.api_base}{endpoint}"
        req = urllib.request.Request(url)
        req.add_header('Authorization', f'DeviceKey {self.device_token}')  # FIXED: was Bearer
    ...
    def _save_cache(self):
        """Save to disk so clients are available immediately on next startup"""
        try:
    ...
    def _load_cache(self):
        """Load from disk on startup (before first network sync fires)"""
