...
PLATFORM NOTE:
  This file is a port of windows_agent/ai_client_switcher.py and the matching
  logic is deliberately identical — the two agents must agree about which
  client a title names, or the same firm gets different answers depending on
  who is at which desk. The differences are confined to five places, each
  marked "MAC:" below:

    * DEFAULT_CONFIG["skip_exes"]  — bundle identifiers, not .exe names
    * _MAC_BUNDLE_FAMILIES         — bundle ids for the exe_family routing rules
    * _should_skip()               — Finder stands in for Explorer
    * _looks_like_path()           — POSIX paths, not drive letters
    * _notify_switch()             — UNUserNotificationCenter, not win10toast

  The Mac agent passes the bundle identifier (com.microsoft.Excel) in the
  parameter called exe_name, because the two agents share this interface.
  Read "exe_name" as "whatever this OS calls the application".
"""
...
_CHROME_ONLY_PATTERNS = [
    ...
    # MAC: the same idea in the Mac vocabulary — an app frontmost with no
    # document open. Finder's own window titles ("Finder", "Recents") name a
    # place, not a client.
    re.compile(r'^\s*Finder\s*$', re.IGNORECASE),
    re.compile(r'^\s*(Recents|AirDrop|iCloud\s+Drive|Macintosh\s+HD)\s*$', re.IGNORECASE),
    re.compile(r'^\s*Preview\s*$', re.IGNORECASE),
    re.compile(r'^\s*(Numbers|Pages|Keynote)\s*$', re.IGNORECASE),
    re.compile(r'^\s*(Mail|Messages|Calendar|Reminders|Notes)\s*$', re.IGNORECASE),
    re.compile(r'^\s*(System\s+Settings|System\s+Preferences)\s*$', re.IGNORECASE),
    re.compile(r'^\s*TextEdit\s*$', re.IGNORECASE),
    re.compile(r'^\s*Untitled(\s+\d+)?\s*$', re.IGNORECASE),
]
...
_PATH_NOISE_FOLDERS = {
    ...
    "c:", "d:", "e:",
    # MAC: the folders a Mac path passes through on its way to the file.
    "volumes", "applications", "system", "private", "var", "usr", "opt",
    "mobile documents", "cloudstorage", "containers",
    "application support", "caches", "movies", "public",
}
...
# MAC: the same families, addressed the way macOS addresses an application.
# An org routing rule is written once by the admin ("exe_family=excel →
# Internal-Bookkeeping") and has to fire on both agents, but the Windows
# prefixes never will: 'com.microsoft.excel'.startswith('excel') is False.
# These are matched as exact bundle ids or as a bundle-id prefix, so
# 'com.microsoft.teams2' and 'com.intuit.quickbooks2024' both land.
_MAC_BUNDLE_FAMILIES = {
    ...
    'zoom':        ['us.zoom.xos'],
    'webex':       ['com.cisco.webexmeetingsapp', 'com.webex.meetingmanager'],
    'acrobat':     ['com.adobe.acrobat', 'com.adobe.reader'],
...
def _exe_family_matches(family_key: str, exe_name: str) -> bool:
    """Given 'taxwise' and 'utw25.exe' — or 'excel' and
    'com.microsoft.Excel' — return True.

    MAC: the Mac agent hands us a bundle identifier in exe_name, so both
    vocabularies are consulted. The Windows prefixes are kept because this
    file is a port and the two must not drift; they simply never match a
    bundle id.
    """
    if not exe_name:
        return False
    family_low = family_key.lower()
    exe_low = exe_name.lower().strip()

    # MAC: bundle identifier (com.microsoft.Excel, us.zoom.xos, ...)
    if '.' in exe_low and not exe_low.endswith('.exe'):
        for bundle in _MAC_BUNDLE_FAMILIES.get(family_low, []):
            if exe_low == bundle or exe_low.startswith(bundle + '.'):
                return True
            # QuickBooks ships as com.intuit.quickbooks2024 — a version glued
            # straight onto the id with no separating dot.
            if exe_low.startswith(bundle) and exe_low[len(bundle):].isdigit():
                return True
        return False

    prefixes = _EXE_FAMILIES.get(family_low, [family_low])
    exe_low = exe_low.replace('.exe', '')
    return any(exe_low.startswith(p) for p in prefixes)


# v1.4.0: OrgRoutingRuleEngine extracted to windows_agent/routing_rules.py
# Re-export for backwards compatibility with any code that imports it from here.
from routing_rules import OrgRoutingRuleEngine, get_routing_engine
# =====================================================================
# Data Structures
# =====================================================================
...
DEFAULT_CONFIG = {
    ...
    # MAC: the Windows agent sets this False because win10toast steals focus
    ...
    # MAC: bundle identifiers, because that is what the Mac agent passes as
    # exe_name. The Windows .exe names were carried over verbatim in the
    # original port and matched nothing on this platform, which meant the Mac
    # agent never skipped its own shell — Spotlight, the Dock and the lock
    # screen were all being read for client names.
    #
    # Deliberately NOT here: com.apple.Preview, com.apple.TextEdit and the
    # iWork apps. Those open the client's actual documents.
    "skip_exes": {
        "com.apple.finder",                  # see _should_skip: paths excepted
        "com.apple.spotlight",
...
class AIClientSwitcher:
    ...
    def __init__(self, config=None, api_base="", api_key="", openai_api_key="",
        ...
        # v1.4.0: singleton shared with inference engine's routing_rule_match collector
    ...
    def on_window_change(self, app_name, exe_name, title, url=None, file_path=None,
                         in_meeting=False, bundle_id=None):
        """v1.4.0: NO-OP. Inference engine now handles all classification per
        event via its own collectors (routing rules, learned patterns, title
        matching, etc.). Method retained so existing call sites don't NameError."""
    ...
    def _notify_switch(self, new_name: str, old_name: str,
        ...
        # MAC: the Windows agent walks win10toast → PowerShell toast → a
        # borderless Tk window, all of which steal focus on this platform or
        # do not exist. A Mac notification never takes focus, so there is one
        # real path plus one fallback.
        if self.notif_manager and hasattr(self.notif_manager, "_send_notification"):
            try:
                self.notif_manager._send_notification(
                    notif_type=None, title=title, body=full_body
                )
                return
            except Exception as e:
                logger.debug(f"[AI-SWITCH] notif_manager failed: {e}")
        ...
        try:
            ...
            request = UNNotificationRequest.requestWithIdentifier_content_trigger_(
                f"ai-switch-{int(time.time())}", content, None
            )
            UNUserNotificationCenter.currentNotificationCenter(
            ).addNotificationRequest_withCompletionHandler_(request, None)
            return
        except Exception as e:
            logger.debug(f"[AI-SWITCH] UNUserNotificationCenter failed: {e}")
    ...
    def _should_skip(self, title: str, exe_name: str) -> bool:
        ...
        if exe_name and exe_name.lower() in self._skip_exes:
            # MAC: Finder is this platform's Explorer, and gets the same
            # exception. finder_watcher dispatches full POSIX paths
            # ("/Users/x/Clients/Varacchi/2024 1040"); a real path is signal.
            # Finder's own chrome ("Documents", "Recents", "AirDrop") does not
            # look like a path, so it still gets skipped.
            if exe_name.lower() == "com.apple.finder" and self._looks_like_path(title):
        ...
        if t in {
            ...
            # MAC: the bare app names a Mac window shows with nothing open.
            "finder", "safari", "preview", "messages", "activity monitor",
            "system settings", "system preferences", "iterm2",
        }:
            return True
    ...
    def _looks_like_path(s: str) -> bool:
        ...
        MAC: a Mac path has no drive letter to key on, so this asks whether
        the string is rooted and has at least one separator — "/Users/dan/
        Clients/Varacchi" yes, "Documents" no. The Windows forms are kept so
        a path arriving from a synced Windows share still reads as one.
        """
        if not s or len(s) < 2:
            return False
        s = s.strip()
        ...
        if s.startswith("\\\\"):
            return True
        # MAC: absolute POSIX path — rooted, and naming something below root.
        # "/" alone, or "/Users", is a place rather than a path to a file.
        if s.startswith("/") and s.count("/") >= 2:
            return True
        # MAC: home-relative path as Finder and the shell both write it.
