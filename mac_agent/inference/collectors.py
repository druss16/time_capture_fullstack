...
def _strip_qb(title: str) -> str:
    ...
    try:
        try:
            ...
            from mac_agent.ai_client_switcher import _strip_qb_screen_bracket
...
def _cpa_file_match_safe(title: str, clients: list,
    ...
    try:
        try:
            ...
            from mac_agent.ai_client_switcher import _cpa_file_match
...
def _widget_title_matches(title, file_path, url, client_name, aliases) -> bool:
    ...
    try:
        try:
            ...
            from mac_agent.widget_state_tracker import WidgetStateTracker
...
_SYSTEM_PATH_SEGMENTS_BLOCKLIST = {
    ...
    # macOS counterparts. Same intent as the Windows entries above: a path
    # segment that names the operating system's furniture must never be read
    # as a client. '/Users/<name>' is already handled by the skip-after-Users
    # rule, which is spelled with forward slashes and so works on both.
    'library', 'applicationsupport', 'application support',
    'applications', 'volumes', 'private', 'var', 'usr', 'opt', 'system',
    'containers', 'group containers', 'groupcontainers',
    'mobile documents', 'mobiledocuments',   # iCloud Drive's on-disk name
    'cloudstorage',                          # /Library/CloudStorage/<provider>
    'caches', 'preferences', 'trash', '.trash',
}
...
def _filter_path_segments(file_path: str) -> List[str]:
    ...
    \\Users\\ or /Users/), and common system folders on both Windows and
    macOS. This prevents false positives where the account name (e.g.
    'mavops') collides with a client name.
    """
    if not file_path:
        return []
...
def collect_routing_rule_evidence(ctx: WindowContext) -> List[Evidence]:
    ...
    mac_agent.routing_rules. Updates from sync flow to both this
    ...
    try:
        ...
        try:
            from mac_agent.routing_rules import get_routing_engine
        except ImportError:
            return []
...
def collect_tax_software_evidence(
    ...
    try:
        try:
            ...
            from mac_agent.tax_software_constants import (
                TAX_SOFTWARE_RETURN_PATTERN,
                TAX_SOFTWARE_TAXWISE_PATTERN,
                GENERIC_TAX_DIALOGS,
            )
