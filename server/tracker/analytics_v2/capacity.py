...
def capacity_hours_map(org, user_ids, start, end) -> dict[int, float]:
    ...
    # Firm-wide working days in the window (schedule − weekends − holidays −
    # summer Fridays). Everyone shares this gross; time off nets it down per user.
    ...
    for uid in user_ids:
        off_hours = 0.0
        for ts, te in to_by_user.get(uid, []):
