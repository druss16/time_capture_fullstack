...
# The nightly `resolve_org_mismatches` task lived here. Removed with the agent
# that ran it: on a real book of business it had one row a quarter to look at,
# and the errors that matter carry no evidence to act on. The DETECTION pass
# above (scan_org_mismatches) stays — that is the backstop that keeps settled
# time from being recorded under the wrong client, and it is the part that
# earns its nightly run.
