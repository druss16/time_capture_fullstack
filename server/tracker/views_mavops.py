...
def mavops_mismatch_drafts(request):
    ...
    """
    from tracker.services.misfile_evidence import drafts_for_blocks
    ...
    return Response({'org_id': int(org_id),
                     'drafts': drafts_for_blocks(int(org_id), block_ids)})


# `mavops_approve_mismatch_drafts` and `mavops_run_mismatch_agent` stood here —
# apply the agent's draft, and sweep-and-act. Removed with the agent. What is
# left is the read-only `/drafts/` endpoint above, which answers "why is this
# row here" for the sweep, and the existing assign / dismiss / reconcile
# endpoints, which are a PERSON deciding.
